...
def do_inference(
    ...
    for batch_idx, sample in enumerate(tqdm(map_loader)):
        ...
        with torch.no_grad():
            ...
            num_pairs = len(sample["anchor"])
            num_pcs = num_pairs * 2

            time_preproc.toc(call_inc=num_pcs)

            time_backbone.tic()
            batch_dict = model(model_in, compute_backbone=True,
                               compute_embeddings=False, metric_head=False, compute_transl=False, compute_rotation=False)
            time_backbone.toc(call_inc=num_pcs)

            time_nvlad.tic()
            batch_dict = model(batch_dict, compute_embeddings=True,
                               compute_backbone=False, metric_head=False, compute_transl=False, compute_rotation=False)
            time_nvlad.toc(call_inc=num_pcs)

            time_reg.tic()
            batch_dict = model(batch_dict, metric_head=True, compute_transl=True, compute_rotation=True,
                               compute_backbone=False, compute_embeddings=False)
            time_reg.toc(call_inc=num_pairs)
    ...
    if save_times_path:
        ...
        time_preproc.save_json(save_times_path / "times_model_preproc.json")
        time_backbone.save_json(save_times_path / "times_model_backbone.json")
        time_reg.save_json(save_times_path / "times_model_reg.json")
        time_nvlad.save_json(save_times_path / "times_model_lcd")
...
def main_process(
    ...
    if run_inference:
        ...
        dataset_for_recall, map_tree_poses, pair_dist = do_inference(device=device, weights_path=weights_path,
                                                                     ...
                                                                     save_times_path=save_times_path,
