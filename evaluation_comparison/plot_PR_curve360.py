...
def compute_PR(pair_dist, poses, map_tree_poses, is_distance=True, ignore_last=False):
    ...
    for i in tqdm(range(100, last)):
        ...
        valid_idxs = list(set(indices[0]) - set(range(min_range, len(dataset_for_recall))))
        if len(valid_idxs) > 0:
            real_loop.append(1)
        else:
            real_loop.append(0)
        ...
        if is_distance:
            ...
            detected_loop.append(pair_dist[i, candidate])
        candidate_pose = dataset_for_recall.poses[candidate][:3, 3]
...
sequence = "360-09"
dataset_path = '/media/RAIDONE/DATASETS/KITTI-360'
device = torch.device('cuda:0')
dataset_for_recall = KITTI3603DPoses(dataset_path, '2013_05_28_drive_0009_sync', train=False,
...
pd_ours = np.load(f'pairs_dist_ours_{sequence}2.npz')['arr_0']
pd_ours2 = np.load(f'pairs_dist_ours_{sequence}_trained2.npz')['arr_0']
