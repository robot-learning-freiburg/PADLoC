...
matplotlib.rcParams.update({
    ...
    "text.usetex": True,
    # 'pgf.rcfonts': False,
    # 'text.latex.preamble': r'\usepackage{amsfonts,newtxtext,newtxmath,helvet,courier}'
    "text.latex.preamble": r"\usepackage{amsfonts,helvet,courier}"
})
...
def plot(curves: List[Curve], *,
    ...
    for c in curves:
        ...
        if box_plot:
            ax.boxplot(np.abs(c.y).T, positions=c.x, widths=10, showmeans=True, meanline=True,
                       showfliers=False,
                       medianprops={"c": "k"},
                       meanprops={"c": "k"},
                       )
    ...
    ax.set_xticks(np.array(sorted(list(ticks))))
    if len(curves) > 1:
        ax.legend()
...
def main(stat_file_path: Path,
    ...
    plots_cfg = [
        Plot("Rotational Error",             "Yaw [º]", "Error [º]",  "pair_err_yaw_deg"),
        Plot("Translational Error",          "Yaw [º]", "Error [m]",   "pair_err_tra"),
...
def cli_args() -> dict:
    ...
    parser.add_argument("--save_path", type=Path,
                        default=Path("/home/arceyd/Documents/Projects/PADLoC/img/yaw_sensitivity_only_rot_only_padloc/"))
