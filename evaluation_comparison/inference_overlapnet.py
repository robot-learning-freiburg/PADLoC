...
def main_process(gpu, weights_path, args):
    ...
    dataset_for_recall = KITTILoader3DPoses(args.data, sequences_validation[0],
                                            os.path.join(args.data, 'sequences', sequences_validation[0], 'poses.csv'),
    ...
    with open(f'/home/cattaneod/CODES/overlapnet_custom/GT/{exp_cfg["test_sequence"]}/GT.pickle', 'rb') as f:
        gts = pickle.load(f)
    pairwise_overlap = np.zeros((len(gts), len(gts)))
    for i in range(len(gts)):
        pairwise_overlap[i, i:] = gts[i][i:, 4]
    test_pair_idxs = np.stack((pairwise_overlap > 0.1).nonzero(), axis=1)
    ...
    if exp_cfg['weight_rot'] > 0. or exp_cfg['weight_transl'] > 0.:
        ...
        yaw_preds[:, :] = -100
        for batch_idx, sample in tqdm(enumerate(RecallLoader), total=(2*len(test_pair_idxs))//exp_cfg['batch_size']):
            ...
            # _, transl_error, yaw_error = test(model, sample, exp_cfg, device, args.num_iters)

            # if rank == 0:
            #     if batch_idx % 20 == 0 and batch_idx != 0:
            #         print('Iter %d / %d, time = %.2f' % (batch_idx,
            #                                              len(RecallLoader),
            #                                              time.time() - start_time))

    with open(f'yaw_preds_{exp_cfg["test_sequence"]}.pickle', 'wb') as f:
    ...
    if exp_cfg['weight_metric_learning'] > 0.:
        for batch_idx, sample in enumerate(RecallLoader):
            ...
            emb_list.append(emb.detach().clone())
