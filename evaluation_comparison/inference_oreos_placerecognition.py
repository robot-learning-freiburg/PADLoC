...
def main_process(gpu, weights_path, args):
    ...
    dataset_map = torch.utils.data.Subset(dataset_for_recall, subsample_idxs[:split_idx])
    dataset_query = torch.utils.data.Subset(dataset_for_recall, test_pair_idxs[:,1])
    ...
    for i in range(emb_list_query.shape[0]):
        current_pose = dataset_for_recall.poses[test_pair_idxs[i, 1]][:3, 3]
        current_pose = torch.tensor(current_pose)
        distances, indices = map_tree.query(emb_list_query[i], k=10)
        for j in range(len(indices)):

            m = subsample_idxs[indices[j]]
            x = dataset_for_recall.poses[m][0, 3]
            y = dataset_for_recall.poses[m][1, 3]
            z = dataset_for_recall.poses[m][2, 3]
