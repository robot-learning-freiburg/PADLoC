...
from panoptic.epsnet2.scripts.load_scan import get_eps_model, scan_to_epsnet, compact_labels
...
def main_process(local_rank, gpu_per_node, num_nodes, node, common_seed, exp_cfg, args):
    global EPOCH
    global_rank = local_rank+(node*gpu_per_node)
    global_world_size = gpu_per_node*num_nodes
    local_world_size = gpu_per_node
    assert num_nodes == 2, "Only two nodes are supported"

    dist.init_process_group(
        backend='nccl',
        init_method='file:///home/cattaneo/temp/sharedfile',
        world_size=global_world_size,
        rank=global_rank
    )
    local_seed = ((common_seed + common_seed ** local_rank) + node) ** 2
    ...
    dist.barrier()
    print(f"Node {node}, local rank {local_rank}, global rank {global_rank}, seed {common_seed}")

    uotnet_partecipating = [i for i in range(gpu_per_node)]
    eps_partecipating = [i+gpu_per_node for i in range(gpu_per_node)]
    global_group = dist.new_group([i for i in range(global_world_size)])
    eps_group = dist.new_group(eps_partecipating)
    uotnet_group = dist.new_group(uotnet_partecipating)
    nccl_groups = []
    for i in range(gpu_per_node):
        nccl_groups.append(dist.new_group([i, i+gpu_per_node]))

    current_date = datetime.now()
    dt_string = current_date.strftime("%d/%m/%Y %H:%M:%S")
    dt_string_folder = current_date.strftime("%d-%m-%Y_%H-%M-%S")

    exp_cfg['effective_batch_size'] = exp_cfg['batch_size'] * local_world_size

    starting_epoch = 1
    if global_rank < gpu_per_node:
        ...
        # sequences_training = ["00"]

        image_height = exp_cfg['image_height']
        image_width = exp_cfg['image_width']
        ...
        if args.dataset == 'kitti':
            ...
            dataset_for_recall = KITTI3603DPoses(args.data, sequences_validation[0],
                                                 train=False, use_semantic=False,
                                                 use_panoptic=False, without_ground=exp_cfg['without_ground'],
                                                 loop_file=exp_cfg['loop_file'])
        ...
        model.train()
        model = DistributedDataParallel(model.to(device), device_ids=[local_rank], output_device=local_rank,
                                        find_unused_parameters=True, process_group=uotnet_group, dim=-1)
        ...
        eps_model.eval()
        post = KNN(19)
        make_panoptic = PanopticPreprocessing(0.5, 0.5, 4096)
        eps_cfg = yaml.safe_load(open('panoptic/epsnet2/scripts/semantic-kitti-ours.yaml', 'r'))

    aaa = torch.tensor([starting_epoch]).to(device).int()
    dist.barrier()
    dist.broadcast(aaa, src=0, group=global_group)
    starting_epoch = aaa.item()
    ...
    for epoch in range(starting_epoch, exp_cfg['epochs'] + 1):
        ...
        batch_in_dataset = torch.tensor([0]).to(device)
        if global_rank < gpu_per_node:
            batch_in_dataset[0] = len(TrainLoader)
        for n_group in range(gpu_per_node):
            dist.broadcast(batch_in_dataset, src=n_group, group=nccl_groups[n_group])
        print("Batch in dataset: ", batch_in_dataset)

        ## Training ##
        if global_rank >= gpu_per_node:
            for batch_idx in range(batch_in_dataset):
                anchor_padded = torch.zeros((exp_cfg['batch_size'], 100000, 4)).to(device)
                positive_padded = torch.zeros((exp_cfg['batch_size'], 100000, 4)).to(device)
                anchor_shapes = torch.zeros((exp_cfg['batch_size'])).int().to(device)
                positive_shapes = torch.zeros((exp_cfg['batch_size'])).int().to(device)
                dist.barrier()
                for n_group in range(gpu_per_node):
                    dist.broadcast(anchor_padded, src=n_group, group=nccl_groups[n_group])
                    dist.broadcast(positive_padded, src=n_group, group=nccl_groups[n_group])
                    dist.broadcast(anchor_shapes, src=n_group, group=nccl_groups[n_group])
                    dist.broadcast(positive_shapes, src=n_group, group=nccl_groups[n_group])
                dist.barrier()
                print(anchor_shapes)
                print(positive_shapes)
                imgs = []
                p_xs = []
                p_ys = []
                proj_ranges = []
                unproj_ranges = []
                n_points = []
                for sample_id in range(anchor_shapes.shape[0]):
                    anchor = anchor_padded[sample_id][:anchor_shapes[sample_id], :]
                ...
                for sample_id in range(anchor_shapes.shape[0]):
                    positive = positive_padded[sample_id][:positive_shapes[sample_id], :]
                ...
                for i, (sem_pred, bbx_pred, cls_pred, obj_pred, msk_pred, sem_logits) in enumerate(zip(
                    ...
                    unproj_argmax = post(proj_ranges[i, :n_points[i]], unproj_ranges[i, :n_points[i]],
                                         proj_argmax, p_xs[i, :n_points[i]], p_ys[i, :n_points[i]], torch.max(proj_argmax)+1)
                    ...
                    pred_np = map(pred_np, eps_cfg['learning_map_inv'])
                    ################################################## Simple Mapping
                    ...
                    dist.barrier()
                    for n_group in range(gpu_per_node):
                        dist.broadcast(unproj_sem_logits, src=gpu_per_node+n_group, group=nccl_groups[n_group])
                        dist.broadcast(unproj_panoptic_logits, src=gpu_per_node+n_group, group=nccl_groups[n_group])
                    dist.barrier()
                    #############################################################
        ...
        if global_rank < gpu_per_node:
            for batch_idx, sample in enumerate(TrainLoader):
                ...
                for sample_id in range(len(sample['anchor'])):
                    a_pad = sample['anchor'][sample_id]
                    p_pad = sample['positive'][sample_id]
                    anchor_shapes[sample_id] = a_pad.shape[0]
                    positive_shapes[sample_id] = p_pad.shape[0]
                    anchor_padded[sample_id] = torch.nn.functional.pad(a_pad, [0, 0, 0, 100000 - a_pad.shape[0]])
                    positive_padded[sample_id] = torch.nn.functional.pad(p_pad, [0, 0, 0, 100000 - p_pad.shape[0]])
                ...
                for sample_id in range(len(sample['anchor'])):
                    unproj_sem_logits = torch.zeros((19, anchor_shapes[sample_id])).to(device)
                    unproj_panoptic_logits = torch.zeros((56, anchor_shapes[sample_id])).to(device)
                    dist.barrier()
                    for n_group in range(gpu_per_node):
                        dist.broadcast(unproj_sem_logits, src=gpu_per_node+n_group, group=nccl_groups[n_group])
                        dist.broadcast(unproj_panoptic_logits, src=gpu_per_node+n_group, group=nccl_groups[n_group])
                    dist.barrier()
                    sample['anchor_logits'].append(torch.cat((unproj_sem_logits, unproj_panoptic_logits)).T.clone())
                for sample_id in range(len(sample['anchor'])):
                    unproj_sem_logits = torch.zeros((19, positive_shapes[sample_id])).to(device)
                    unproj_panoptic_logits = torch.zeros((56, positive_shapes[sample_id])).to(device)
                    dist.barrier()
                    for n_group in range(gpu_per_node):
                        dist.broadcast(unproj_sem_logits, src=gpu_per_node+n_group, group=nccl_groups[n_group])
                        dist.broadcast(unproj_panoptic_logits, src=gpu_per_node+n_group, group=nccl_groups[n_group])
                    dist.barrier()
                    sample['positive_logits'].append(torch.cat((unproj_sem_logits, unproj_panoptic_logits)).T.clone())
                print("ciao")
                ...
                if local_rank == 0:
                    loss = (loss / local_world_size).item()
                    loss_rot = (loss_rot / local_world_size).item()
                    loss_transl = (loss_transl / local_world_size).item()
                    local_loss += loss
                    local_iter += 1
        ...
        if exp_cfg['weight_rot'] > 0. or exp_cfg['weight_transl'] > 0.:
            for batch_idx, sample in enumerate(TestLoader):
                ...
                if local_rank == 0:
                    transl_error = (transl_error / local_world_size).item()
                    yaw_error = (yaw_error / local_world_size).item()
        ...
        if exp_cfg['weight_metric_learning'] > 0.:
            for batch_idx, sample in enumerate(RecallLoader):
                ...
                dist.barrier()
                out_emb = [torch.zeros_like(emb) for _ in range(local_world_size)]
                # print(f'{rank} {emb}')
                ...
                #     dist.gather(emb)
                if local_rank == 0:
                    interleaved_out = torch.empty((emb.shape[0]*local_world_size, emb.shape[1]),
                                                  device=emb.device, dtype=emb.dtype)
                    for current_rank in range(local_world_size):
                        interleaved_out[current_rank::local_world_size] = out_emb[current_rank]
    ...
    if exp_cfg['scheduler'] == 'swa' and local_rank == 0:
        ...
        best_model = {
            'config': exp_cfg,
            'epoch': epoch,
            'state_dict': swa_model.module.module.state_dict(),
            'optimizer': optimizer.state_dict(),
        }
        torch.save(best_model, savefilename)
...
if __name__ == '__main__':
    ...
    if args.gpu == -1:
        mp.spawn(main_process, nprocs=args.gpu_per_node, args=(args.gpu_per_node, args.num_nodes, args.node, 42, cfg['experiment'], args,))
    else:
        main_process(args.gpu, args.gpu_per_node, args.num_nodes, args.node, 42, cfg['experiment'], args)
