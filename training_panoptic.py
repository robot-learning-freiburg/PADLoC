...
from utils.tools import _pairwise_distance, run_panoptic
...
def main_process(local_rank, gpu_per_node, num_nodes, node, common_seed, exp_cfg, args):
    ...
    dist.init_process_group(
        backend='nccl',
        init_method=args.nccl_init,
        world_size=global_world_size,
        rank=global_rank
    )
    local_seed = (common_seed + common_seed ** global_rank) ** 2
    ...
    exp_cfg['effective_batch_size'] = exp_cfg['batch_size'] * global_world_size
    ...
    for epoch in range(starting_epoch, exp_cfg['epochs'] + 1):
        ...
        for batch_idx, sample in enumerate(TrainLoader):
            ...
            sample = run_panoptic(eps_model, sample, device, make_panoptic, post, exp_cfg)
            ...
            if global_rank == 0:
                loss = (loss / global_world_size).item()
                loss_rot = (loss_rot / global_world_size).item()
                loss_transl = (loss_transl / global_world_size).item()
                local_loss += loss
                local_iter += 1
        ...
        if exp_cfg['weight_rot'] > 0. or exp_cfg['weight_transl'] > 0.:
            for batch_idx, sample in enumerate(TestLoader):
                ...
                if global_rank == 0:
                    transl_error = (transl_error / global_world_size).item()
                    yaw_error = (yaw_error / global_world_size).item()
        ...
        if exp_cfg['weight_metric_learning'] > 0.:
            for batch_idx, sample in enumerate(RecallLoader):
                ...
                dist.barrier()
                out_emb = [torch.zeros_like(emb) for _ in range(global_world_size)]
                # print(f'{rank} {emb}')
                ...
                #     dist.gather(emb)
                if global_rank == 0:
                    interleaved_out = torch.empty((emb.shape[0]*global_world_size, emb.shape[1]),
                                                  device=emb.device, dtype=emb.dtype)
                    for current_rank in range(global_world_size):
                        interleaved_out[current_rank::global_world_size] = out_emb[current_rank]
...
if __name__ == '__main__':
    ...
    current_date = datetime.now()
    dt_string_folder = current_date.strftime("%d-%m-%Y_%H-%M-%S")
    nccl_init = f'file:///home/cattaneo/temp/{dt_string_folder}'
    args.nccl_init = nccl_init
