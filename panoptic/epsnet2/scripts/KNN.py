...
#     print("Cleaning point-clouds with kNN post-processing")
    self.knn = 5 #params["knn"]
    self.search = 5 #params["search"]
    self.sigma = 3.0 #params["sigma"]
    self.cutoff = 1.0 #params["cutoff"]
  ...
  def __call__(self, proj_range, unproj_range, proj_argmax, px, py, nclasses):
...
#    print ('knn_ag', knn_argmax[0,:,1745],knn_argmax_onehot[0,:,1745]) 
    # now vote (as a sum over the onehot shit)  (don't let it choose unlabeled OR invalid)
    knn_argmax_out = knn_argmax_onehot[:, :-1].argmax(dim=1) # + 1
#    print ('knn_final_prediction', knn_argmax_out[0,1745], knn_invalid_idx[0,:,1745]) 
