...
class ISSTransform:
    ...
    def _random_flip(img, msk):
        if random.random() < 0.5:
            ...
            #img[:,:,3:] = img[:,::-1,] #img.transpose(Image.FLIP_LEFT_RIGHT)
            #img = np.fliplr(img[])
...
#        print (h,w,th,tw)
        #if w < tw:
        #    img = np.pad(img, (tw - w, 0), 0, 'constant')
        ...
        img = img[:,i:i+th,j:j+tw].contiguous() #tfn.crop(img, i, j, th, tw)
...
#            target_size = self.shortest_size
        ...
        #scale = self._adjusted_scale(2048, 64, )
        

#        out_size = tuple(int(dim * scale) for dim in img.size)
#        img = img.resize(out_size, resample=Image.BILINEAR)
        msk = [m.resize((img.shape[2],img.shape[1]), resample=Image.NEAREST) for m in msk]
...
#        print ('final', img.shape, msk.shape)
