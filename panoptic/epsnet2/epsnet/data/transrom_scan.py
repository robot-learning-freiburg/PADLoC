...
#        self.upsample = torch.nn.Upsample(size=(256,4096),mode='bilinear')    
        #self.u_upsample = torch.nn.Upsample(size=(256,4096),mode='nearest')    
    ...
    @staticmethod
    def _compact_labels(msk, cat, track_id, iscrowd):
        ids = np.unique(msk)
        if 0 not in ids:
...
#        print ('labia', len(iscrowd),len(cat),len(track_id),len(ids))
...
#            print (l,j)
    ...
    def __call__(self, img, msk, cat, track_id, iscrowd, proj_msk):
        # Random flip
#        if self.random_flip:
...
#        img = self.upsample(img.unsqueeze(0)).squeeze(0)

#        out_size = tuple(int(dim * scale) for dim in img.size)
#        for i in range(5): 
#            k = (img[i,:,:].numpy())
#            k = np.uint8(255*(k/np.max(k)))
#            cv2.imwrite('/home/valada/mohan/img'+str(i+6)+'.png', cv2.applyColorMap(k,cv2.COLORMAP_JET))

#        print ('shape', k.shape, img.shape) 
        #cv2.waitKey(0)
        
#        img = img.resize(out_size, resample=Image.BILINEAR)
#        print ('before',np.unique(np.array(msk[0])))
#        msk = [m.resize((4096,256), resample=Image.NEAREST) for m in msk]
#        print ('after',len(np.unique(np.array(msk[0]))))
        # Random Crop
#        if self.random_crop:
...
#        k = np.array(msk[0])
#        k = np.uint8(255*k/np.float(np.max(k))) 
#        cv2.imwrite('/home/valada/mohan/img.png', cv2.applyColorMap(k,cv2.COLORMAP_JET))

        msk, cat, track_id, iscrowd = self._compact_labels(msk, cat, track_id, iscrowd)
        ...
        track_id = torch.from_numpy(track_id.astype(np.long))
        iscrowd = torch.from_numpy(iscrowd)
        bbx = extract_boxes(msk, cat.numel())
        return dict(img=img, msk=msk, cat=cat, iscrowd=iscrowd, bbx=bbx, track_id=track_id, proj_msk=proj_msk)
...
class ISSTestTransform:
    ...
    def __call__(self, img, proj_msk):
        # Adjust scale
...
#        img = self._normalize_image(img)
        ...
        return dict(img=img, proj_msk=proj_msk)
