...
class ISSTestDataset_(data.Dataset):
    ...
    def __init__(self, in_dir, transform):
        ...
        for img_path in chain(
                *(glob.iglob(path.join(self.in_dir, '**', ext), recursive=True) for ext in ISSTestDataset_._EXTENSIONS)):
...
#                "size": size,
    ...
    def img_sizes(self):
        """Size of each image of the dataset"""
        return [(256,4096) for img_desc in self._images]
...
#        print (proj.shape)
        ...
        # rec = self.transform(proj, proj_mask)
        rec = {}
        rec["img"] = proj
        rec["proj_msk"] = proj_mask
...
#            img = self.transform(img_raw.convert(mode="RGB"))
        ...
        rec["idx"] = scan_desc["idx"]
        rec["size"] = size
        rec["rel_path"] = path.relpath(self._images[item]["path"], self.in_dir)
        rec["abs_path"] = self._images[item]["path"] 
        return rec
