...
class ARBatchSampler(Sampler):
    ...
    def _generate_batches(self):
        ...
        self.img_sets[0] = [self.img_sets[0][i] for i in range(len(self.img_sets[0]))]
        self.img_sets[1] = [self.img_sets[1][i] for i in range(len(self.img_sets[1]))]
        ...
        for img_set in self.img_sets:
            ...
            for img in img_set:
                #img_ = img.copy()
#                for tframe in reversed(range(self.tframes)):
#                    img_['id']=img['id']-tframe
#                    print (img_)
                batch.append(img)
...
class DistributedARBatchSampler(ARBatchSampler):
    ...
    def __iter__(self):
        ...
        g = torch.Generator()
        g.manual_seed(self._epoch)
        indices = list(range(len(batches))) #list(torch.randperm(len(batches), generator=g))

        # add extra samples to make it evenly divisible
        indices += indices[:(self.num_batches * self.num_replicas - len(indices))]
        #print(len(indices),3*self.num_batches * self.num_replicas, batches[1])
        ...
        for idx in indices:
            batch = batches[idx] #sorted(batches[idx], key=lambda i: i["ar"])
            batch = [i["id"] for i in batch]
            yield batch
