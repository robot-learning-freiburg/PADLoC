...
class ISSDataset(data.Dataset):
    ...
    def _load_item(self, item):
        ...
        proj_x[:unproj_n_points] = (torch.from_numpy(scan.proj_x)/2048)*3072
        proj_y = torch.full([self.max_points], -1, dtype=torch.long)
        proj_y[:unproj_n_points] = (torch.from_numpy(scan.proj_y)/64)*128
...
#        print (proj.shape)
        ...
        while path.exists("{}.{}".format(msk_file, i)):
            ...
            i += 1
        del proj_x,proj_y,proj_xyz, proj_range, proj_remission,unproj_xyz, unproj_range, unproj_remissions, scan
        cat = scan_desc["cat"]
        iscrowd = scan_desc["iscrowd"]
        track_id = scan_desc["iscrowd"]
        return proj, msk, cat, track_id, iscrowd, proj[0].clone(), scan_desc["id"]
    ...
    def __getitem__(self, item):
        #print ('item', item)
        img, msk, cat, track_id, iscrowd, proj_msk, idx = self._load_item(item)
#        print (proj_msk.shape)
