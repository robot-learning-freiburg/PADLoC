...
class LaserScan:
  ...
  def __init__(self, project=False, H=289, W=4097, fov_up=3.0, fov_down=-25.0):
  ...
  def open_scan(self, filename):
    ...
    jump = jump = scan[:,4]
    self.set_points(points, remissions,jump)


  def _transorm_train(self, depth, refl, labels, py, px, points_xyz):
    new_h = 289
    new_w = 4097
  ...
  def set_points(self, points, remissions=None,jump =None ):
  ...
  def do_range_projection(self):
    ...
    #######################GET ROWS##########################
    #yaw = np.arctan2(scan_z, depth)
    #jump= np.absolute(yaw[1:]-yaw[:-1]) > np.radians(0.3)
    #print(np.sum(jump))
    #print(jump.shape)
    #jump = np.argwhere(jump> 0.00523599)
    #print(jump.shape,"))))))))))))))))))))))))")
  ...
  ##############################################################################
    ...
    proj_x = (0.5 * (yaw / np.pi + 1.0))
    proj_y = -self.jump.astype(np.int32)         # in [0.0, 1.0]
    proj_x = proj_x -0.25
    #print(np.max(proj_x),np.min(proj_x))
    ppp = np.where(proj_x < 0)
    proj_x[ppp] = 1+ proj_x[ppp]
    #print(np.max(proj_x),np.min(proj_x))
    proj_x = (proj_x * self.proj_W - 0.001).astype(np.int32)
    ...
    abc = self._transorm_train(self.proj_range, self.proj_remission, None, proj_y, proj_x, points[:, :3])
    #print(abc[0].shape, np.min(abc[0]))
    depth = abc[0]
    depth = (depth/np.amax(depth))*255
    #print(depth.shape,np.amax(depth),np.amin(depth))
    im_color = cv2.applyColorMap(np.uint8(depth), 16)


    #cv2.imshow('spherical_image', im_color)
...
class SemLaserScan(LaserScan):
  ...
  def __init__(self,  sem_color_dict=None, project=False, H=32, W=2048, fov_up=3.0, fov_down=-25.0, max_classes=300):
