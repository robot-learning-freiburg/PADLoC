...
from epsnet.utils.bbx import calculate_shift
...
class InstanceSegAlgo:
    ...
    def _match_to_lbl(self, proposals, bbx, cat, ids, track_ids, msk, match):
        cls_lbl = []
        bbx_lbl = []
        msk_lbl = []
        track_lbl = []
        for i, (proposals_i, bbx_i, cat_i, ids_i, track_i, msk_i, match_i) in enumerate(zip(
                proposals, bbx, cat, ids, track_ids, msk, match)):
            if match_i is not None:
                ...
                #track_lbl_i = proposals_i.new_zeros(proposals_i.size(0), dtype=torch.long)
                #track_lbl_i[pos] = track_i[match_i[pos]]
                # Bounding box regression labels
                if pos.any().item():
                    track_lbl_i = track_i[match_i[pos]] 
                    bbx_lbl_i = calculate_shift(proposals_i[pos], bbx_i[match_i[pos]])
#                    print ('aamo', i, match_i[pos], bbx_i[match_i[pos]])
                    bbx_lbl_i *= bbx_lbl_i.new(self.bbx_reg_weights)
                    #print ('ahaha', bbx_lbl_i) 
                    ...
                    msk_lbl_i = None
                    track_lbl_i = None
                track_lbl.append(track_lbl_i) 
                cls_lbl.append(cls_lbl_i)
                bbx_lbl.append(bbx_lbl_i)
                msk_lbl.append(msk_lbl_i)
            else:
                track_lbl.append(None)
                cls_lbl.append(None)
                bbx_lbl.append(None)
                msk_lbl.append(None)

        return PackedSequence(cls_lbl), PackedSequence(bbx_lbl), PackedSequence(msk_lbl), PackedSequence(track_lbl)
