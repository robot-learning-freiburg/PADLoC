...
#                    continue 
           ...
           for idx in range(cls_pred_.shape[0]):
                y0 = int(bbx_pred_[idx,0]*scale)
                y1 = int((bbx_pred_[idx,2]*scale).round()+1)
                x0 = int(bbx_pred_[idx,1]*scale)
                x1 = int((bbx_pred_[idx,3]*scale).round()+1)
           ...
           mask_energy = mask_energy*4+seg_inst_logits #(((mask_energy*4).sigmoid()+seg_inst_logits.sigmoid()))*(4*mask_energy+seg_inst_logits)
