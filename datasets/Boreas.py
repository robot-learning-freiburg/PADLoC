...
class Boreas(Dataset):
	...
	def __init__(self, dir, sequence, without_ground=False, train=False):
		...
		base2lidar = np.loadtxt(os.path.join(dir, sequence, 'calib', 'T_applanix_lidar.txt'))
		# base2lidar = np.eye(4, dtype=np.float64)
		poses = os.path.join(dir, sequence, 'applanix', 'lidar_poses.csv')
		poses2 = []
		self.stamps = []
		with open(poses, 'r') as f:
			...
			for x in f:
				...
				poses2.append(pose)
		lidar_files = os.listdir(os.path.join(dir, sequence, 'lidar'))
	...
	def get_velo(self, idx, dir, sequence, without_ground, jitter=False):
		if without_ground:
			...
			velo_path = os.path.join(dir, sequence, 'lidar', f'{self.lidar_stamps[idx]}.bin')
			scan = np.fromfile(velo_path, dtype=np.float32).reshape((-1, 6))
		scan = scan[:,:4]
		scan[:, -1] /= 255.
...
if __name__ == '__main__':
	dataset1 = Boreas('/home/cattaneo/Datasets/boreas', 'boreas-2020-12-18-13-44')
	dataset3 = Boreas('/home/cattaneo/Datasets/boreas', 'boreas-2020-11-26-13-58')
